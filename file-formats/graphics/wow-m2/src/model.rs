...
impl M2Model {
    ...
    pub fn parse<R: Read + Seek>(reader: &mut R) -> Result<Self> {
        ...
        // Parse non-essential sections — failures are tolerated so that
        // models with partially-implemented structures (e.g. particle emitters)
        // still load with geometry and textures intact.
        let particle_emitters = read_array(reader, &header.particle_emitters.convert(), |r| {
            M2ParticleEmitter::parse(r, header.version)
        })
        .unwrap_or_else(|e| {
            eprintln!("[M2] warning: failed to parse particle emitters: {}, skipping", e);
            Vec::new()
        });
        ...
        // Collect raw animation keyframe data — also fault-tolerant
        ...
        let raw_data = M2RawData {
            ...
            .unwrap_or_default(),
            bounding_triangles: read_raw_bytes(reader, &header.bounding_triangles, 2)
                .unwrap_or_default(),
            bounding_vertices: read_raw_bytes(reader, &header.bounding_vertices, 12)
                .unwrap_or_default(),
            bounding_normals: read_raw_bytes(reader, &header.bounding_normals, 12)
                .unwrap_or_default(),
